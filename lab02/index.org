...
Generating random numbers in any computer is hard. The theory was laid down long ago in Knuth's Art's of Computer Programming. But in security, random numbers are an important building block.
...
the results. For this we are going to use =R=. We will only plot the first 1000 values.  Create two file called =random.R= and =urandom.R= by reusing the
following script. Replace =data= with =random= and =urandom= to process the corresponding files.  Each script create two png files, the first with the points
...
- Explain why reading from =/dev/urandom= is significantly slower than =/dev/random/=.
