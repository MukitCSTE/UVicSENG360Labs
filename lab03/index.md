...
Investigate the traﬃc you have captured. Remember that you can filter the traffic in the filter window (for example by the protocol type `ftp` or by IP address `ip.addr == XXXX`). Can you find the password in the traffic?
...
Start capturing network traffic. On the same Firefox, refresh only the tab on [https://mitmproxy.org/](https://mitmproxy.org/). Once the page has loaded, stop capturing network traffic.
...
With Firefox still configured to talk to the mitmproxy, try visiting/refreshing the wikipedia tab: [https://www.wikipedia.org/](https://www.wikipedia.org/). You should see a warning like before.
