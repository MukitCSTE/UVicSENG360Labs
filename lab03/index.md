...
- You will need the file `mitm-proxy-1.0.tar.gz`. You can get it in our shared folder at `/seng/seng360`, download it from CourseSpaces or from [https://crypto.stanford.edu/ssl-mitm/](https://crypto.stanford.edu/ssl-mitm/).
	- I recommend copying it from our shared folder into your home directory with the `cp` command.
...
Investigate the traﬃc you have captured. Remember that you can filter the traffic in the filter window (for example by IP address `ip.addr == XXXX` or by the protocol type). Can you find the password in the traffic?
...
Use command line to login - something like this: `sftp seng360sftp`. Then enter the provided credentials. Stop capturing the network traffic after you login.

**Question 2:** Investigate the traffic you have captured. Were you able to intercept the password? What happened in the SFTP traffic? How was the secured communications channel established?

# Part 3: Detecting MITM Attacks #

We are now going to create a Man In The Middle attack. Start up Wireshark on one of your console terminals. Then open up Firefox on the same machine and visit [https://www.wikipedia.org/](https://www.wikipedia.org/). Once the page has loaded, stop capturing network traffic.

**Question 3:** Since you are using a secure HTTPS connection, take a look at how it is secured. In Firefox, inspect the certificate. Who is verifying Wikipedia?

---

For the next step, you will need the file `mitm-proxy-1.0.tar.gz`. You can download it from our shared folder, CourseSpaces, or from [https://crypto.stanford.edu/ssl-mitm/](https://crypto.stanford.edu/ssl-mitm/). I recommend copying it from our shared folder into your home directory with the `cp` command.

Download and extract the file to your home directory:

`tar -zxvf mitm-proxy-1.0.tar.gz`

Run the proxy with the supplied script by going into the extracted folder's directory and running the script with admin privileges:
...
- Click OK to apply.
- **NOTE 1: Remember to undo this proxy setting when you are done with the lab!**
- *Note 2:You may need to restart the proxy server multiple times since it is prone to crashing and displaying java stack traces.*

Start capturing network traffic. Open up Firefox on the same machine and re-visit [https://www.wikipedia.org/](https://www.wikipedia.org/) again. Once the page has loaded, stop capturing network traffic.

- We recommend that you **do not** login to the Wikipedia site (if you have an account there).
...
**Question 4:** Suppose you ignored the untrusted certificate warning and logged in to the site. What are the potential consequences of this?  
**Question 5:** Characterize/explain the MITM attack you performed. How can MITM attacks be mitigated?  
**Question 6:** How do modern browsers "know" to trust a certificate from the server?  
	- Hint: Lookup [Certificate Authorities](https://en.wikipedia.org/wiki/Certificate_authority) on Wikipedia and give a brief explanation in your own words. Does this remind you of a certain concept from Lab 02?
