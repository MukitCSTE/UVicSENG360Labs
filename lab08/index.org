...
many programmers do not check whether the data structures that they allocate in the stack are overflown.
...
*Q1. What is the warning you receive when you compile it? Based on the behaviour of this program, explain why it is warning.*
...
the addresses that we need to override (when we try to attack a binary we can't compile). Let us disable it by running it as follows:
...
This is a very well crafted piece of code. The challenge of the attack is to get the address of the string "/bin/sh" in the =rdi= register. But we have no clue
...
*Q7. List the 3 counter-measures against stack overflows that we learned in this lab*
